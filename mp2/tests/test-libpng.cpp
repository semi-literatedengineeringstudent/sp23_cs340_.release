...
TEST_CASE("`PNG_open` validates a valid PNG signature", "[weight=1][part=1]") {
  ...
  PNG *png;
  REQUIRE( (png = PNG_open("TEST_340.png", "r")) != NULL );
  PNG_close(png);
  system("rm -f TEST_340.png");
}
...
TEST_CASE("`PNG_read` returns the correct return values", "[weight=5][part=1]") {
  system("cp tests/files/340.png TEST_340.png");
...
TEST_CASE("`PNG_open` writes a correct PNG header", "[weight=1][part=1]") {
  PNG *png;
...
TEST_CASE("`PNG_write` writes a PNG chunk and CRC", "[weight=5][part=1]") {
  PNG *png;
